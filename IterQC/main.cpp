...
void shrink_graph(MiniGraph& mini_g,int d_limit){
    ...
    for(ui i = 0;i < n;i ++) {
        ui u, key;
        heap->pop_min(u, key);
        if(key>=d_limit) break;
        peel_sequence[i] = u;
        for(int j=h[u];j!=-1;j=ne[j]) heap->decrement(e[j],1);
    ...
    if(new_graph!=nullptr) delete new_graph;
    new_graph=new Graph();
    ...
    vector<int> vset;
    for(int i=0;i<n;i++)
        if(!mini_g.is_del(i)) deg[idx]=mini_g.get_deg(i),vset.push_back(i),id_map[i]=idx++;
    pstart[0]=0;
    for(int i=1;i<=new_n;i++) pstart[i]=pstart[i-1]+deg[i-1];
...
int main(int argc, char *argv[]) {
    ...
    cout<<"Extract UB:"<<(double)(clock()-start)/CLOCKS_PER_SEC<<"s UB value:"<<n<<" LB value:"<<
    lb<<endl;
    ...
    if(lb>0) shrink_graph(mini_g,my_floor((lb-1)*gama)+1); 
    cout<<"Prunning graph:"<<endl<< "new n:"<<mini_g.n<<" "<<"new m:"<<mini_g.m<<endl;
    ...
    if(need_iteration){
        ...
        while(1){
            ...
            if(k==my_floor(alpha*(n-1)+1)&&!is_bad_instance) break;
