...
public:
    ...
    void init_with_basic(int n_,int m_,ui* d_,ui* pstart_,ui* edge_to_){
        n=n_;
        m=m_;
        pstart=new ui[n+1];
        edge_to=new ui[m];
        ...
        memcpy(edge_to,edge_to_,m*sizeof(int));
        memcpy(d,d_,n*sizeof(int));
        map_refresh_id.resize(n);
        for (ui i = 0; i < n; i++) map_refresh_id[i] = i;
    }

    void copy_graph(Graph* g){
        init_with_basic(g->n,g->m,g->d,g->pstart,g->edge_to);
    }
    ...
    int degen_for_ego(ui range, vector<vector<ui>> &neighbor, vector<ui> &res)
    {
        ...
        for (ui i = 0; i < range; i++) deg[i] = neighbor[i].size();
