...
void heuris()
{
    input_n = g.n;
    FastHeuris();
    if(use_strong_heuris) StrongHeuris(); // generate n subgraphs and compute larger lb
}
...
{   
    ...
    lb=2*k-2;
    solution.clear();
    for(int i=0;i<lb;i++) solution.insert(i);
...
#ifndef NO_FAKE_LB
    int fake_lb=(lb+UB)/2;
    if(k==my_floor((1-gama)*(fake_lb-1)+1)||UB<lb) fake_lb=lb;
    cout<<"fake_lb:"<<fake_lb<<endl;
    if(lb<fake_effect) fake_effect=true;
    lb=fake_lb;
#else
    int fake_lb=lb;
#endif
    ...
    if(heuris_average > bnb_average) use_strong_heuris=false;
    else use_strong_heuris=true;
#endif
